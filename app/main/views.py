...
def edit_avatar():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            im.save(os.path.join(current_app.config['UPLOAD_FOLDER'], 
                                   'avatar', filename))
            flash('Upload file successfully.')
...
def uploaded_file(filename):
    return send_from_directory(
        os.path.join(current_app.config['UPLOAD_FOLDER'], 'avatar'), 
        filename)
'''

IMAGE_FOLDER = 'avatar'
UPLOAD_FOLDER = os.path.join(r'E:\other\flasky\app\upload', IMAGE_FOLDER)

@main.route('/user/<username>/edit_avatar')
...
def upload():
    if request.form.get("action") == "add":
        ...
        imgfile=os.path.join(UPLOAD_FOLDER, "{}.jpg".format(user))
